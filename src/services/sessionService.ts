...
export const sessionService = {
  ...
  async checkPersistentSession(): Promise<boolean> {
    ...
    if (timeSinceLastCheck < 10000 && localStorage.getItem('is_authenticated') === 'true') {
      console.log(
        '🔒 Using cached auth status (checked ' + (timeSinceLastCheck / 1000).toFixed(1) + 's ago)'
      );
      return true;
    }
    ...
    if (timeSinceLastCheck < backoffTime) {
      console.log(
        `🔒 Rate limiting auth check, last check was ${(timeSinceLastCheck / 1000).toFixed(1)}s ago (waiting ${(backoffTime / 1000).toFixed(1)}s)`
      );
      ...
      try {
        ...
        try {
          ...
          if (status && status.authenticated) {
            ...
            console.log(
              '🔒 Auth status says not authenticated but marked locally - trying refresh'
            );
        ...
        if (this._checkAttempts >= 5) {
          console.warn(
            `🔒 Too many session check failures (${this._checkAttempts}), clearing authentication state`
          );
          localStorage.removeItem('is_authenticated');
        }
