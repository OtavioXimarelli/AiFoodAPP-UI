...
export const useLocalStorage = <T>(
  ...
  const saveData = useCallback((newItems: T | T[]) => {
    try {
      setError(null);
  ...
  const updateItem = useCallback((itemId: number | string, updatedItem: Partial<T>) => {
    try {
      setError(null);
  ...
  const deleteItem = useCallback((itemId: number | string) => {
    try {
      setError(null);
  ...
  const getSortedData = useCallback((sortBy: 'createdAt' | 'updatedAt' = 'createdAt') => {
  ...
  // Search data
  const searchData = useCallback((searchTerm: string, searchFields: string[] = ['name', 'title']) => {
    if (!searchTerm.trim()) return data;
    
    const lowerSearchTerm = searchTerm.toLowerCase();
    return data.filter(item => 
      searchFields.some(field => 
        (item as any)[field]?.toLowerCase().includes(lowerSearchTerm)
      )
    );
  }, [data]);
