...
const NutritionAnalysisModal = ({ open, onOpenChange, recipe, analysis }: NutritionAnalysisModalProps) => {
  ...
  const isAnalysisSaved = storedAnalyses.some(saved => 
          ...
          Object.keys(sugestoes).forEach(key => {
            if (key !== 'fontes_de_proteina_magra' && key !== 'graos_integrais') {
              ...
              if (typeof value === 'string') {
                ...
                structured.outrasRecomendacoes.push(`${key.replace(/_/g, ' ')}: ${value.join(', ')}`);
              }
            }
          });
        }
  ...
  const handleSaveAnalysis = async () => {
    ...
    try {
      const analysisToSave: NutritionAnalysis = {
        ...
        recommendations: [...structuredAnalysis.sugestoes, ...structuredAnalysis.fontesProteina, ...structuredAnalysis.graosIntegrais],
        insights: [...structuredAnalysis.outrasRecomendacoes, structuredAnalysis.observacoes].filter(Boolean),
  ...
  const nutritionMetrics = [
    {
      icon: Zap,
      label: "Energia",
      value: structuredAnalysis.calorias || `${nutritionValues.calorias || recipe.calories || 'N/A'} kcal`,
              ...
              <CardContent className="p-6">
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
                  ...
                  <div className="flex-1">
                    <h3 className="text-xl sm:text-2xl font-bold text-foreground mb-3">{recipe.name}</h3>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {nutritionMetrics.map((metric, index) => (
                  <Card key={index} className="border border-border/50 hover:border-primary/30 bg-gradient-to-br from-background to-muted/20 hover:shadow-xl transition-all duration-300 group overflow-hidden">
                    <CardContent className="p-6 text-center space-y-4 relative">
                      <div className="absolute inset-0 bg-gradient-to-br from-transparent via-transparent to-primary/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                      <div className="flex items-center justify-between relative z-10">
                        <div className={`p-3 ${metric.bgColor} rounded-2xl group-hover:scale-110 transition-transform duration-300 shadow-sm`}>
                      ...
                      <div className="space-y-3 relative z-10">
                        <p className="text-2xl sm:text-3xl font-bold text-foreground group-hover:scale-105 transition-transform duration-300">{metric.value}</p>
                ...
                <Card className="border border-border/50 bg-gradient-to-br from-background to-muted/20 shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden">
                  ...
                  <CardContent className="space-y-4 p-6">
                    {structuredAnalysis.outrasRecomendacoes.map((point, index) => (
                      <div key={index} className="flex items-start gap-4 p-5 bg-gradient-to-r from-blue-50/70 to-indigo-50/50 dark:from-blue-950/20 dark:to-indigo-950/10 rounded-2xl border border-blue-100 dark:border-blue-800/30 hover:shadow-md transition-all duration-300 group">
                        ...
                        <p className="text-sm text-foreground leading-relaxed flex-1 font-medium">{point}</p>
                      </div>
                    ))}
                  </CardContent>
                </Card>
                ...
                <Card className="border border-border/50 bg-gradient-to-br from-green-50/70 to-emerald-50/50 dark:from-green-950/20 dark:to-emerald-950/10 shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden">
                  ...
                  <CardContent className="space-y-4 p-6">
                    {structuredAnalysis.sugestoes.map((suggestion, index) => (
                      <div key={index} className="flex items-start gap-4 p-5 bg-gradient-to-r from-green-50/80 to-emerald-50/60 dark:from-green-950/30 dark:to-emerald-950/20 rounded-2xl border border-green-100 dark:border-green-800/30 hover:shadow-md transition-all duration-300 group">
                        ...
                        <p className="text-sm text-foreground leading-relaxed flex-1 font-medium">{suggestion}</p>
                      </div>
                    ))}
                  </CardContent>
                </Card>
              ...
              <Card className="border border-border/50 bg-gradient-to-br from-amber-50/70 to-orange-50/50 dark:from-amber-950/20 dark:to-orange-950/10 shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden">
                ...
                <CardContent className="p-6">
                  <div className="flex items-start gap-4 p-5 bg-gradient-to-r from-amber-50/80 to-orange-50/60 dark:from-amber-950/30 dark:to-orange-950/20 rounded-2xl border border-amber-100 dark:border-amber-800/30 hover:shadow-md transition-all duration-300 group">
                    ...
                    <p className="text-sm text-foreground leading-relaxed flex-1 font-medium">{structuredAnalysis.observacoes}</p>
                  </div>
                </CardContent>
              </Card>
            )}
