...
export const MorphButton = memo<MorphButtonProps>(
  ({ children, className, onClick, variant = 'primary', size = 'md', disabled = false }) => {
...
export const AnimatedProgress = memo<AnimatedProgressProps>(
  ({ value, max = 100, className, showValue = false, color = 'primary' }) => {
...
export const AnimatedText = memo<AnimatedTextProps>(
  ({ text, className, delay = 0, speed = 0.05, effect = 'typewriter' }) => {
    const { shouldReduceMotion } = useOptimizedAnimation();
...
export const AnimatedList = memo<AnimatedListProps>(
  ({ children, className, staggerDelay = 0.1, variant = 'slideInUp' }) => {
    const { shouldReduceMotion } = useOptimizedAnimation();
    ...
    return (
      ...
      >
        {React.Children.map(children, (child, index) => (
          <motion.div key={index} variants={enhancedVariants[variant]}>
            {child}
          </motion.div>
        ))}
      </motion.div>
...
export const PageTransitionWrapper = memo<PageTransitionWrapperProps>(({ children, className }) => {
  const { shouldReduceMotion } = useOptimizedAnimation();
