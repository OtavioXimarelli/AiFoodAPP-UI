...
export const OptimizedImage = memo<OptimizedImageProps>(
  ({
    ...
    if (!priority && !hasIntersected) {
      return (
        <div ref={setRefs} className={`${className} bg-muted`} style={{ width, height }}>
          {placeholder === 'skeleton' && <SkeletonPulse className="w-full h-full" />}
...
export const ImageGallery = memo<ImageGalleryProps>(
  ({ images, className = '', itemClassName = '', columns = 3, gap = 8, priority = 3 }) => {
    const gridStyle = {
      display: 'grid',
...
export const ProgressiveImage = memo<ProgressiveImageProps>(
  ({ src, srcSet, sizes, alt, className = '', ...props }) => {
...
export const OptimizedAvatar = memo<OptimizedAvatarProps>(
  ({ src, alt = 'Avatar', fallback, size = 'md', className = '' }) => {
    ...
    if (!src || imageError) {
      return (
        <div
          className={`${sizeClasses[size]} ${className} bg-muted rounded-full flex items-center justify-center`}
        >
