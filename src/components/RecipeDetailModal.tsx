...
import { X, Clock, Users, Flame, ChefHat, List, ShoppingCart, Calendar, Star } from 'lucide-react';
                  ...
                  <div className="flex flex-wrap gap-2">
                    {recipe.tags.map((tag, index) => (
                      <Badge
                        key={index}
                        variant="secondary"
                        className="px-3 py-1 text-sm bg-white/95 border border-border/30"
                      >
                        {tag}
                      </Badge>
                    ))}
                ...
                <CardContent>
                  <div className="grid gap-3 sm:grid-cols-2">
                    {recipe.ingredientsList.map((ingredient, index) => (
                      <div
                        key={index}
                        className="group flex items-center gap-4 p-4 bg-gradient-to-r from-background to-muted/30 rounded-xl border border-border/20 hover:border-border/40 transition-all duration-200 hover:shadow-md"
                      >
                ...
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                    {recipe.nutritionalInfo.map((info, index) => (
                      <div
                        key={index}
                        className="p-3 bg-gradient-to-br from-background to-muted/30 rounded-xl border border-border/20 text-center group hover:shadow-md transition-all duration-200"
                      >
                        <Badge
                          variant="outline"
                          className="w-full text-sm py-2 px-3 group-hover:border-primary/30 transition-colors"
                        >
                          {info}
                        </Badge>
                      </div>
